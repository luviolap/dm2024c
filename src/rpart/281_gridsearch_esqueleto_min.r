...
for (cp in c(-0.5, 0)) { #-1, -0.5, 0, 0.5, 1
  for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {
    for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {
      for (minbucket in c(5, 10)) { #5, 10, 20, 30
        # notar como se agrega
        # logging: print current parameters
        cat(sprintf("Testing: cp=%f, maxdepth=%d, minsplit=%d, minbucket=%d\n", 
                    cp, vmax_depth, vmin_split, minbucket))
