...
if __name__ == "__main__":
    ...
    # Create directory
    os.makedirs("~/buckets/b1/exp/EX2330", exist_ok=True)
    os.chdir("~/buckets/b1/exp/EX2330")

    # Use multiprocessing to call ArbolEstimarGanancia for each seed
    with Pool(cpu_count()) as pool:
        salidas = list(tqdm(pool.imap(lambda x: ArbolEstimarGanancia(x, PARAM), PARAM["semillas"]), total=len(PARAM["semillas"])))

    # Convert the list of dictionaries to a Polars DataFrame
    tb_salida = pl.DataFrame(salidas)

    # Save results
    tb_salida.write_csv("tb_salida.txt", separator="\t")

    # Plot density
    plt.figure(figsize=(10, 6))
    sns.kdeplot(tb_salida['ganancia_test'], shade=True)
    
    mean = tb_salida['ganancia_test'].mean()
    quantiles = tb_salida['ganancia_test'].quantile([0.05, 0.5, 0.95])
    
    plt.axvline(mean, color='red', linestyle='--')
    for q in quantiles:
        plt.axvline(q, color='blue', linestyle=':')
    
    plt.title('Density of ganancia_test')
    plt.xlabel('ganancia_test')
    plt.ylabel('Density')
    plt.savefig('densidad.pdf')
    plt.close()
    ...
    for q in [1, 2, 4, 8, 16, 32, 64, 128]:
        ...
        grupos = 0
        while i + q - 1 < cant:
            gan = tb_salida['ganancia_test'][i:i+q].mean()
            vgan.append(gan)
            i += q
            grupos += 1
        ...
        tb_final.append({
            'grupo_size': q,
            'grupos': grupos,
            'gan_media': np.mean(vgan),
            'gan_sd': np.std(vgan)
        })

    tb_final = pl.DataFrame(tb_final)
    print(tb_final)